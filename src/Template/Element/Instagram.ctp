...
<article id="SNS_Instagram">
    <header>
		...
		<h1>
            ...
            <div>
					<a href="https://www.instagram.com/<?php echo $ig_data->username; ?>/" style="<?= $is_link_disable ? 'pointer-events:none; ' : null ?>" target="new" rel="noopener">
					<?php echo $ig_data->name."\n"; ?>
					(@<?php echo $ig_data->username; ?>)
					</a>
            </div>
        </section>
        <nav>
    ...
    <input type="radio" name="show-mode" id="show_list" value="list"<?php if(API['INSTAGRAM_SHOW_MODE'] === 'list'){ echo ' checked'; } ?>>
