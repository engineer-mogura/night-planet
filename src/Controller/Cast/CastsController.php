...
class CastsController extends AppController
{
    ...
    public function beforeFilter(Event $event)
    {
        ...
        $this->Security->setConfig('unlockedActions', ['saveProfile','saveTopImage','saveGallery','deleteGallery'
        , 'saveDiary', 'deleteDiary', 'updateDiary', 'sns', 'editCalendar']);
    ...
    public function index()
    {
        ...
        $cast = $this->Casts->find('all')
            ->contain(['Shops', 'CastLikes' => function ($q) use ($id){
                return $q
                    ->select(['CastLikes.cast_id'
                        , 'total' => $q->func()->count('CastLikes.cast_id')])
    ...
    public function editCalendar()
    {
        ...
        try {
            ...
            $cast_schedule = $this->CastSchedules->find('all', array('fields' => $Columns))
                ->where(['shop_id' => $this->viewVars['castInfo']['shop_id']
                    , 'cast_id' => $this->viewVars['castInfo']['id']]);
            ...
            if (!empty($file["name"]) && $file["name"] == 'blob') {
                ...
                try {
                    ...
                    $upResult = $this->S3Rapper->upload(
                        $this->viewVars['castInfo']['icon_path'] . DS . $convertFile, $file["tmp_name"]);
    ...
    public function topImage()
    {
        ...
        $files = $this->S3Client->getList($this->s3Backet,
             $this->viewVars['castInfo']['top_image_path'], 1);
        // ファイルが存在したら、画像をセット
        if (is_countable($files) ? count($files) > 0 : 0) {
    ...
    public function saveTopImage()
    {
        ...
        if (!empty($file["name"]) && $file["name"] == 'blob') {
            ...
            try {
                ...
                // 画像ファイルアップロード
                $upResult = $this->S3Rapper->upload(
                    $this->viewVars['castInfo']['top_image_path'] . DS . $convertFile, $file["tmp_name"]);
                // 同じファイル名でない場合は前の画像を削除
            ...
            foreach ($files as $key => $file) {
                ...
                if (!empty($file["name"]) && $file["name"] == 'blob') {
                    ...
                    $convertFile = $this->Util->file_upload($file, $files_befor, $chkDuplicate
                        , $this->viewVars['castInfo']['image_path'], $limitFileSize);
                    ...
                    $result = $this->S3Rapper->upload(
                        $this->viewVars['castInfo']['image_path'] . DS . $convertFile, $file["tmp_name"]);
                }
            }
    ...
    {
        $allDiary = $this->getAllDiary($this->viewVars['castInfo']['id']
            , $this->viewVars['castInfo']['diary_path'], null);
        ...
        try {
            ...
            foreach ($files as $key => $file) {
                ...
                if (!empty($file["name"]) && $file["name"] == 'blob') {
                    ...
                    // ファイル名を取得する
                    $convertFile = $this->Util->file_upload($file, $files_befor, $chkDuplicate
                        , $this->viewVars['castInfo']['diary_path'] . $diary->dir, $limitFileSize);
                    ...
                    $result = $this->S3Rapper->upload(
                        $this->viewVars['castInfo']['diary_path'] . $diary->dir . DS . $convertFile, $file["tmp_name"]);
    ...
    public function deleteDiary()
    {
        ...
        try {
            ...
            // 一時ディレクトリ作成
            $tmpDir = new Folder(
                WWW_ROOT.$this->viewVars['castInfo']['tmp_path'] . DS . time(), true, 0777);
            // 一時ディレクトリにバックアップ実行
