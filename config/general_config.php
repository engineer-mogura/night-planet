...
if (strpos(env('APP_URL'), 'local') !== false) {
...
} else if (strpos(env('APP_URL'), '192.168.33.10') !== false) {
    // スマホのローカル環境の場合
    $URL_S3_BUCKET = env('AWS_URL_HOST') . DS . env('AWS_BUCKET');
    define('AWS_S3_CONFIG', array(
        'credentials' => [
...
} else if (strpos(env('APP_URL'), 'work') !== false) {
    // ステージング環境の場合
    ...
    define('API', array(
        ...
        'INSTAGRAM_BUSINESS_ID' => env('INSTAGRAM_BUSINESS_ID'), // INSTAGRAMビジネスアカウントID
        'INSTAGRAM_GRAPH_API_ACCESS_TOKEN' => env('INSTAGRAM_GRAPH_API_ACCESS_TOKEN'), // #3INSTAGRAMアクセストークン
        'INSTAGRAM_GRAPH_API' => 'https://graph.facebook.com/v11.0/', // インスタグラムのAPIパス
        'INSTAGRAM_MAX_POSTS' => 9, // インスタグラムの最大投稿取得数
        'INSTAGRAM_SHOW_MODE' => 'grid', // インスタグラム表示モード
        'INSTAGRAM_CACHE_TIME' => 360, // インスタグラムキャッシュタイム
    ));
...
return [
    ...
    define('PATH_ROOT', array(
        ...
        'NIGHT_PLANET_LOGO' => $URL_S3_BUCKET . '/others/NightPlanet_logo.png',
        'NIGHT_PLANET_LOGO_ICON' => $URL_S3_BUCKET . '/others/NightPlanet_logo_icon.png',
    ...
    define('RESULT_M', array(
        ...
        'AUTO_FREE_PLAN_CHANGE_SUCCESS' => '有料プランの期限を過ぎましたので、フリープランへ自動移行しました。',
